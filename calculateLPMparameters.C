...
void calculateLPMparameters(Float_t phantomSize = 200, Int_t eventsToUse = 1000, Float_t spotSize = -1, Int_t material = kWater);
...
void calculateLPMparameters(Float_t phantomSize, Int_t eventsToUse, Float_t spotSize, Int_t material) {
   ...
   if (spotSize <0) { // spotsize not defined -> it's 3 mm
   ...
   // define parameter limits for matrix search
   Float_t  AXlow = 0.9;
   Float_t  AXhigh = 1;
   Float_t  APlow = 0.35;
   Float_t  APhigh = 0.45;
   ...
   for (Int_t i=0; i<tree->GetEntries(); ++i) {
      ...
      if (lastEID != eventID) {
         ...
         if (hResidualEnergy->GetEntries() > 25)   energyFilter = hResidualEnergy->GetMean() * 0.9;
         else                                      energyFilter = 0;
